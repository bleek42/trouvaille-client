...
export default function Interests() {
  ...
  const interestsArray = ['Camping', 'Hiking', 'Beaches', 'Parks', 'Zoos', 'Museums', 'Breweries', 'Wineries', 'Amusement', 'Haunted', 'Novelty', 'Memorial', 'Monuments'];
      ...
      <div className="interests-container">
        <form>
          {interestsArray.map((interest, idx) => (
            <label key={idx} htmlFor={interest}>{interest}
              <input type="checkbox" name={interest} id={interest} onChange={handleCheckBox} />
            </label>
          ))}
        </form>
      </div>
